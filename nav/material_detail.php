...
<?php 
    ...
    $_SESSION["SAPNumber"] = $_GET['SAPNumber'];
    $sessionValue = $_SESSION["SAPNumber"];

?>
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Materiál <?php echo $sessionValue; ?></title>
...
<?php session_start(); ?>
    ...
    <main>
        <?php
            ...
            // Basic info
            $sql = "SELECT SAPNumber, MaterialName, MaterialType, MaterialCategory, MaterialNote, SupplierName FROM `agc_Material` INNER JOIN `agc_Supplier` ON agc_Material.SupplierIDFK = agc_Supplier.SupplierID WHERE SAPNumber = '$sessionValue' LIMIT 10";
            ...
            if ($result->num_rows > 0) {
                while($row = mysqli_fetch_assoc($result)) {
                    ...
                    if($row["MaterialNote"]){
                        $materialSap = $sessionValue;
                        $materialName = $row["MaterialName"];
                        $materialType = $row["MaterialType"];
                        $materialCategory = $row["MaterialCategory"];
                        $materialNote = $row["MaterialNote"];
                        $materialSupplier = $row["SupplierName"];

                        $_SESSION["SAPNumber"] = $sessionValue;
                        $_SESSION["MaterialName"] = $materialName;
                        $_SESSION["MaterialType"] = $materialType;
                        $_SESSION["MaterialCategory"] = $materialCategory;
                        $_SESSION["MaterialNote"] = $materialNote;
                        $_SESSION["SupplierName"] = $materialSupplier;
        ...
        <div class="detail-container">
            ...
            <div class="detail-info">
                <?php
                if(isset($materialNote)){
                    ?>
                    <div class="detail-heading">
                        <div class="detail-name">
                            <h1><?php echo $materialName ?></h1>
                            <h2>SAP: <?php echo $materialSap ?></h2>
                        </div>
                        <div class="detail-edit">
                            <?php
                            if(isset($_SESSION["login"]) && $_SESSION["login"] == true){
                            ?>
                                <a href="./material_update.php">
                    ...
                    <div class="detail-des">
                        <p>Typ: <?php echo $materialType ?></p>
                        <p>Kategorie: <?php echo $materialCategory ?></p>
                        <p>Dodavatel: <?php echo $materialSupplier ?></p>
                        <p>Poznámka: <?php echo $materialNote ?></p>
                    </div>
                    <?php
                }
                else{
            ...
            <div class="detail-materials split">
                <div class="section-1">
                    <div class="table-heading">
                        <h2>Využíván</h2>
                    </div>
                    <div class="result-wrapper project-detail">
                        <div class="parameters split">
                            <p>Projekt / Technický list</p>
                        </div>
                        <div class="result" id="result">
                        <?php
                            //Nalezeno v projektech/listech
                            $sql2 = "SELECT DISTINCT(ProjectName), ProjectID, TechnicalSheetNumber FROM agc_Material INNER JOIN agc_MaterialPosition ON agc_Material.SAPNumber = agc_MaterialPosition.SAPNumberFK INNER JOIN agc_TechnicalSheet ON agc_TechnicalSheet.TechnicalSheetNumber = agc_MaterialPosition.TechnicalSheetNumberFK INNER JOIN agc_Project ON agc_Project.ProjectID = agc_TechnicalSheet.ProjectIDFK WHERE agc_Material.SAPNumber = '$sessionValue' OR agc_Material.MaterialName = '$sessionValue'";
 
                            $result2 = mysqli_query($conn, $sql2);
                            if($result2->num_rows > 0){
                                while($row2 = mysqli_fetch_assoc($result2)){
                                    
                                    if(isset($row2["TechnicalSheetNumber"])){
                                        echo "<a class=\"hint\" href=\"sheet_detail.php?TechnicalSheetNumber=".$row2["TechnicalSheetNumber"]."\"><div class=\"results split\"><p>".$row2["TechnicalSheetNumber"]."</p></div></a>";
                                    }
                                    if(isset($row2["ProjectID"])){
                                        echo "<a class=\"hint\" href=\"project_detail.php?ProjectID=".$row2["ProjectID"]."\"><div class=\"results split\"><p>".$row2["ProjectName"]."</p></div></a>";
                                    }
                                    else{
                ...
                <div class="section-2">
                    ...
                    <div class="result-wrapper project-detail">
                        <div class="parameters">
                            <p>CAS</p>
                            <p class="chemicalName">Název</p>
                            <p>Obsah</p>
                        </div>
                        <div class="result" id="result">
                        <?php 
                            //Chemické složení
                            $sql3 = "SELECT agc_MaterialChemicalComponent.SAPNumberFK, agc_ChemicalComponent.CASNumber, agc_ChemicalComponent.ChemicalComponentName, agc_MaterialChemicalComponent.ShareWeight, agc_ChemicalComponent.ChemicalComponentNote FROM agc_ChemicalComponent INNER JOIN agc_MaterialChemicalComponent ON agc_ChemicalComponent.ChemicalComponentID = agc_MaterialChemicalComponent.ChemicalComponentIDFK WHERE agc_MaterialChemicalComponent.SAPNumberFK = '$sessionValue'";
                            ...
                            if ($result3->num_rows > 0) {
                                while($row3 = mysqli_fetch_assoc($result3)) {
                                    if($row3["CASNumber"]){
                                        ...
                                        echo "<a class=\"hint\" href=\"component_detail.php?CASNumber=".$row3["CASNumber"]."\"><div class=\"results\"><p>".$row3["CASNumber"]."</p><p class=\"chemicalName\">".$row3["ChemicalComponentName"]."</p><p>".$row3["ShareWeight"]."</p></div></a>";
                                    }
                                    else{

                                        echo "<a class=\"hint\" href=\"component_detail.php?ChemicalComponentName=".$row3["ChemicalComponentName"]."\"><div class=\"results\"><p></p><p class=\"chemicalName\">".$row3["ChemicalComponentName"]."</p><p>".$row3["ShareWeight"]."</p></div></a>";
                                    }
                                }
                            }
                            else {
