...
<head>
    ...
    <title>Aktualizace dodavatele</title>
        ...
        <div class="container ">
            ...
            <div class="add-section">
                <div class="heading add">
                    <h1>Aktualizace dodavatele</h1>
                </div>
                <div class="add-form">
                    <p>Formulář dodavatele</p>
                    <form action="" method="post">
                    <?php
                    function Connect(){
                    ...
                    $sql2 = "SELECT SupplierName FROM agc_Supplier WHERE SupplierID = '$GETsupplierID'";

                    $result2 = mysqli_query($conn, $sql2);
                    if ($result2->num_rows > 0) {
                        while($row2 = mysqli_fetch_assoc($result2)) {
                
                            $GETsupplierName = $row2["SupplierName"];
                        }
                    }
                    else {
                    ...
                    if(!isset($_POST["supplierUpdateSubmit"])){
                        ?>
                        
                            <input type="text" name="supplierName" placeholder="Název dodavatele" value="<?php echo $GETsupplierName ?>">
                            <input type="submit" name="supplierUpdateSubmit" value="Aktualizovat">
                            
                        <?php
                    }
                    if(isset($_POST["supplierUpdateSubmit"]) && !empty($_POST["supplierName"]) && strlen($_POST["supplierName"]) <= 64){
    
                        $supplierName = $_POST["supplierName"];
    
                        $supplierName = strip_tags(htmlentities(mysqli_real_escape_string($conn, $supplierName)));
    
    
                        $sql = "UPDATE agc_Supplier SET SupplierName = '$supplierName' WHERE SupplierID = '$GETsupplierID'";
        
                        $result = mysqli_query($conn, $sql);
                        mysqli_close($conn);
                        echo "<p class=\"add-success\">Dodavatel ".$supplierName." byl úspěšně aktualizován.</p>";
                    }
                    else{
                        
                    }
                   
                    if(isset($_POST["supplierUpdateSubmit"]) && (strlen($_POST["supplierName"]) > 64)){
                        ...
                        ?>
                            ...
                            <input type="text" name="supplierName" placeholder="Název dodavatele" value="<?php if(isset($supplierName)){ echo $supplierName; } ?>">
                            <input type="submit" name="supplierUpdateSubmit" value="Aktualizovat">
                        
                        <?php
                        echo "<p class=\"add-error\">Byl překročen počet znaků u názvu dodavatele.</p>";
                    }
                    if(isset($_POST["supplierUpdateSubmit"]) && empty($_POST["supplierName"])){
                        ...
                        $sql = "UPDATE agc_Supplier SET SupplierName = '$GETsupplierName' WHERE SupplierID = '$GETsupplierID'";
        
                        $result = mysqli_query($conn, $sql);
                        mysqli_close($conn);
                        echo "<p class=\"add-success\">Dodavatel ".$supplierName." nebyl aktualizován.</p>";
                    }
                    ?>
                    </form>
                </div>
