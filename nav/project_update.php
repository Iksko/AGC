...
<head>
    ...
    <title>Aktualizace projektu</title>
    ...
    <main>
        <div class="container ">
            ...
            <div class="add-form">
                ...
                ?>
                <?php
                    if(!isset($_POST["projectUpdateSubmit"])){
                        ?>
                        <form action="" method="post">
                        <input type="text" name="projectName" placeholder="Název projektu *" value="<?php echo $_SESSION["ProjectName"]; ?>">
                        <div class="input-split">
                            <input type="text" name="projectShortcut" placeholder="Zkratka projektu *" value="<?php echo $_SESSION["ProjectShortcut"]; ?>">
                            <input type="text" name="projectImds" placeholder="IMDS projektu" value="<?php echo $_SESSION["IMDSName"]; ?>">
                        </div>
                        <input type="text" name="projectCustomer" placeholder="Zákazník *" value="<?php echo $_SESSION["Customer"]; ?>">
                        <input type="submit" name="projectUpdateSubmit" value="Aktualizovat">
                    <?php
                    }
                    if(isset($_POST["projectUpdateSubmit"])){
                        ...
                        $projectID = strip_tags(htmlentities(mysqli_real_escape_string($conn, $projectID)));
                        ...
                        else if(empty($_POST["projectShortcut"])){
                            $projectShortcut = $_SESSION["ProjectShortcut"];
                        }
                        else if(empty($_POST["projectImds"])){
                            $projectImds = $_SESSION["IMDSName"];
                        }
                        else if(empty($_POST["projectCustomer"])){
                            $projectCustomer = $_SESSION["Customer"];
                        }
                    ?>

                    </form>
                    <?php
                        if(isset($_POST["projectUpdateSubmit"]) && strlen($projectName) > 64 || strlen($projectShortcut) > 8 || strlen($projectCustomer) > 64 || strlen($projectImds) > 8){
                            ?>
                            <form action="" method="post">
                                <input type="text" name="projectName" placeholder="Název projektu *" value="<?php echo $_SESSION["ProjectName"]; ?>">
                                <div class="input-split">
                            ...
                            $sql = "UPDATE agc_Project SET ProjectName = '$projectName', ProjectShortcut = '$projectShortcut', IMDSName = '$projectImds', Customer = '$projectCustomer' WHERE ProjectID = '$projectID'";
        
                            $result = mysqli_query($conn, $sql);
                            mysqli_close($conn);
                            echo "<form action=\"\" method=\"post\"><p class=\"add-success\">Projekt ".$projectName." byl úspěšně aktualizován.</p></form>";
                        }
                    }
                    else{
