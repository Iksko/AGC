    ...
    <main>
        <div class="container ">
            ...
            <div class="heading add">
                <h1>Přidání projektu</h1>
            </div>
            <div class="add-form">
                <p>Formulář projektu</p>
                <form action="" method="post">
                <?php
                function Connect(){
                ...
                if(!isset($_POST["projectAddSubmit"]) && (empty($_POST["projectName"]) || empty($_POST["projectShortcut"]) || empty($_POST["projectCustomer"]) || empty($_POST["projectImds"]))){
                    ?>
                        <input type="text" name="projectName" placeholder="Název projektu *">
                        <div class="input-split">
                            <input type="text" name="projectShortcut" placeholder="Zkratka projektu *">
                            <input type="text" name="projectImds" placeholder="IMDS projektu *">
                        </div>
                        <input type="text" name="projectCustomer" placeholder="Zákazník *">
                        <input type="submit" name="projectAddSubmit" value="Přidat">
                    <?php
                }
                else{
                ...
                if(isset($_POST["projectAddSubmit"]) && (empty($_POST["projectName"]) || strlen($_POST["projectName"]) > 64 || empty($_POST["projectShortcut"]) || strlen($_POST["projectShortcut"]) > 8 || empty($_POST["projectCustomer"]) || strlen($_POST["projectCustomer"]) > 64 || strlen($_POST["projectImds"]) > 8)) {

                        $conn = Connect();

                        $projectName = $_POST["projectName"];
                        $projectShortcut = $_POST["projectShortcut"];
                        $projectCustomer = $_POST["projectCustomer"];
                        $projectImds = $_POST["projectImds"];

                        $projectName = strip_tags(htmlentities(mysqli_real_escape_string($conn, $projectName)));
                        $projectShortcut = strip_tags(htmlentities(mysqli_real_escape_string($conn, $projectShortcut)));
                        $projectCustomer = strip_tags(htmlentities(mysqli_real_escape_string($conn, $projectCustomer)));
                        $projectImds = strip_tags(htmlentities(mysqli_real_escape_string($conn, $projectImds)));
                    ?>
                        
                        <input type="text" name="projectName" placeholder="Název projektu *" value="<?php if(isset($projectName)){echo $projectName;}else{} ?>">
                        <div class="input-split">
                            <input type="text" name="projectShortcut" placeholder="Zkratka projektu *" value="<?php if(isset($projectShortcut)){echo $projectShortcut;}else{} ?>">
                            <input type="text" name="projectImds" placeholder="IMDS projektu *" value="<?php if(isset($projectImds)){echo $projectImds;}else{} ?>">
                        </div>
                        <input type="text" name="projectCustomer" placeholder="Zákazník *" value="<?php if(isset($projectCustomer)){echo $projectCustomer;}else{} ?>">
                        <input type="submit" name="projectAddSubmit" value="Přidat">
                    <?php
                ...
                if(isset($_POST["projectAddSubmit"]) && (!empty($_POST["projectName"]) && strlen($_POST["projectName"]) <= 64) && (!empty($_POST["projectShortcut"]) && strlen($_POST["projectShortcut"]) <= 8) && (!empty($_POST["projectCustomer"]) && strlen($_POST["projectCustomer"]) <= 64) && (strlen($_POST["projectImds"]) <= 8)){
                    ...
                    $sql = "INSERT INTO agc_Project (ProjectName, ProjectShortcut, IMDSName, Customer) VALUES ('$projectName', '$projectShortcut', '$projectImds', '$projectCustomer')";
    
                    $result = mysqli_query($conn, $sql);
                    echo "<p class=\"add-success\">Projekt ".$projectName." byl úspěšně přidán.</p>";
                ...
                ?>
                    ...
                    <?php
                        if(isset($_POST["projectAddSubmit"]) && (empty($_POST["projectName"]) || strlen($_POST["projectName"]) > 64 || empty($_POST["projectShortcut"]) || strlen($_POST["projectShortcut"]) > 8 || empty($_POST["projectCustomer"]) || strlen($_POST["projectCustomer"]) > 64 || (strlen($_POST["projectImds"]) > 8))){
                            ...
                            if(empty($_POST["projectName"]) || empty($_POST["projectShortcut"]) || empty($_POST["projectCustomer"])){
                                
                                echo "<p class=\"add-error\">Nebyla zadána všechna povinná pole.</p>";
                            }
                            else if(strlen($_POST["projectName"]) > 64){

                                echo "<p class=\"add-error\">Byl překročen počet znaků u názvu projektu.</p>";
                            }
                            else if(strlen($_POST["projectShortcut"]) > 8){
                                echo "<p class=\"add-error\">Byl překročen počet znaků u zkratky projektu.</p>";
                            }
                            else if(strlen($_POST["projectImds"]) > 8){
                                echo "<p class=\"add-error\">Byl překročen počet znaků u IMDS projektu.</p>";
                            }
                            else if(strlen($_POST["projectCustomer"]) > 64){
                                echo "<p class=\"add-error\">Byl překročen počet znaků u zákazníka projektu.</p>";
                            }
                            else{
