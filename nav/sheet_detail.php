...
<?php 
    ...
    $_SESSION["TechnicalSheetNumber"] = $_GET['TechnicalSheetNumber'];
    $sessionValue = $_SESSION["TechnicalSheetNumber"];

?>
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Technický list <?php echo $sessionValue; ?></title>
...
<?php session_start(); ?>
    ...
    <main>
        <?php  
            ...
            // Basic info
            $sql = "SELECT TechnicalSheetNumber, GlassType, LampTemp, Market, `Check` FROM `agc_TechnicalSheet` WHERE TechnicalSheetNumber = '$sessionValue' LIMIT 10";
            ...
            if ($result->num_rows > 0) {
                while($row = mysqli_fetch_assoc($result)) {
                    ...
                    if($row["Check"] == 'Ano'){
                        $sheetNumber = $row["TechnicalSheetNumber"];
                        $sheetType = $row["GlassType"];
                        $sheetLampTemp = $row["LampTemp"];
                        $sheetMarket = $row["Market"];
                        $sheetCheck = "Potřeba upravit";
                        $sheetCheckStatus = $row["Check"];

                        $_SESSION["TechnicalSheetNumber"] = $sheetNumber;
                        $_SESSION["GlassType"] = $sheetType;
                        $_SESSION["LampTemp"] = $sheetLampTemp;
                        $_SESSION["Market"] = $sheetMarket;
                        $_SESSION["Check"] = $sheetCheckStatus;
                    ...
                    if(isset($sheetCheck)){
                        ...
                        <div class="detail-heading">
                            <div class="detail-name">
                                <h1>Technický list <?php echo $sheetNumber ?></h1>
                                <h2>Typ skla: <?php echo $sheetType ?></h2>
                            </div>
                            <div class="detail-edit">
                                <?php
                                if(isset($_SESSION["login"]) && $_SESSION["login"] == true){
                                ?>
                                    <a href="./sheet_update.php">
                        ...
                        <div class="detail-des">
                            <p><?php echo $sheetLampTemp ?></p>
                            <p>Trh: <?php echo $sheetMarket ?></p>
                            <p><?php echo $sheetCheck ?></p>
                        </div>
                        <?php
                    }
                    else{
